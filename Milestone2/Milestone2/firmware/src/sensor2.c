...
{
    BaseType_t err_code;
    err_code = xQueueSendToBack( sensor2Data.local_q, &millisecondsElapsed,
                                   portMAX_DELAY );
    if(err_code == pdTRUE)
        return 0;
...
void sensor2SendSensorValToSensorQ(unsigned char sensorValue)
{
    ...
    xQueueSendFromISR( sensor2Data.sensor2_q, &sensorValue,
                                   NULL );
#ifdef MACRO_DEBUG
    debugChar(0x02);
#endif
...
/*******************************************************************************
  Function:
    void SENSOR2_Initialize ( void )

  Remarks:
    See prototype in sensor2.h.
 */

void SENSOR2_Initialize ( void )
{
    /* Place the App state machine in its initial state. */
    sensor2Data.state = SENSOR2_STATE_INIT;
    
    /* TODO: Initialize your application's state machine and other
     * parameters.
     */
    //Create the queue
    sensor2Data.local_q = xQueueCreate(10, sizeof(unsigned int));
    //Ensure queue was created. If not, do not continue and turn on LED
    if(sensor2Data.local_q == 0)
    {
        stopEverything();
    }
    sensor2Data.sensor2_q = xQueueCreate(100, sizeof(unsigned char));
    if(sensor2Data.sensor2_q == 0)
    {
        stopEverything();
    }
    //stopEverything();
    //Create the timer
    sensor2Data.local_timer = xTimerCreate( "50msTimer",
    ...
    if(sensor2Data.local_timer == 0)
    {
        stopEverything();
    }
    BaseType_t started = xTimerStart(sensor2Data.local_timer, 0);
    ...
    sensor2Data.state = SENSOR2_STATE_OUTPUT;
}


/******************************************************************************
  Function:
    void SENSOR2_Tasks ( void )
...
void SENSOR2_Tasks ( void )
{
    ...
    switch ( sensor2Data.state )
    {
        /* Application's initial state. */
        case SENSOR2_STATE_INIT:
        {
            break;
        }
...
debugChar(0x08);
            ...
            unsigned int ms;
            BaseType_t received = xQueueReceive(sensor2Data.local_q , &ms, portMAX_DELAY);
            //If not received, stop and turn on LED.
            if(received == pdFALSE)
            {
                stopEverything();
